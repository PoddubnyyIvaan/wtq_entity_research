...
Ссылка для comparison: silver acceptable RU forms (уровни L1/L2 из Stage 6)
и manual-40 (single-rater). Файл: `results/accuracy_metrics_exp2.json`.

| Метрика | Результат |
|---|---|
| Free-generation hit-rate по silver-формам (A/B/C/D) | **0.189 / 0.194 / 0.162 / 0.189** (7/37 и т.п.) — низкий по построению: модель перефразирует WD/PN формы |
| Manual-40 accuracy (mode D, strict: semantic_ok ∧ ¬harmful) | **62.5%** (25/40) |
| accepted без ревью (manual-40) | 45% (18/40) |
| harmful substitutions (manual-40) | 27.5% (11/40) |
| is_entity accuracy vs проектные страты (mode D, n=196) | **75.5%** |
| entity_type exact agreement (n=196) | 31.1% |
| Judge (needs_review) agreement с manual-40 | **55%** (22/40) — совпадение chance-level |

Трактовка: точность перевода free-gen по silver-references ≈ 19% при всех
режимах (контекст не повышает surface-hit rate); нot «accuracy» на judge —
нельзя: это agreement-метрика. Резкий вывод этапа: LLM не компенсирует
слабый entity resolution и добавляет harmful substitutions — что и
привело к Stage 6 (candidate-constrained selection).
